...
export class PrescriptionComponent implements OnInit {
  ...
  constructor(
    ...
    this.patientForm = this._formBuilder.group({
      ...
      //, Validators.pattern('^(((0[1-9]|[12][0-9]|30)[-/]?(0[13-9]|1[012])|31[-]?(0[13578]|1[02])|(0[1-9]|1[0-9]|2[0-8])[-]?02)[-](?:19|20)[0-9]{2}|29[-]?02[-](?:19|20)[0-9]{2})$')]
    });
  }
  ...
  async ngOnInit() {
    ...
    if (this.RestPatientService.profilExist) {
      this.id = Number(localStorage.getItem('profil')!);
      (await this.RestPatientService.getOnePatient(this.id)).subscribe(
        (response) => {
          ...
          localStorage.setItem('Pid', response[0]['id']),
            ...
            localStorage.setItem(
              'BirthDate',
              this.datePipe.transform(response[0]['birth_date'], 'yyyy-MM-dd')!
            ),
  ...
  async openDialog(): Promise<void> {
    ...
    (await this.RestPatientService.getPatients()).subscribe((x) => {
  ...
  async AddPatient() {
    ...
    (await this.AddPatientService.AddPatient(data)).subscribe(
      (x) => {
        this.sucessNotification();
        localStorage.setItem('Pid', JSON.stringify(x[0]['id']));
        localStorage.setItem('BirthDate', x[0]['birth_date']);
      },
      async (err) => {
        console.log(err);
        (await this.RestPatientService.getOnePatientExistant(data.firstName, data.lastName, data.birth_date)).subscribe(x=>{
          localStorage.setItem('Pid', JSON.stringify(x[0]['id']));
          localStorage.setItem('BirthDate', x[0]['birth_date']);
  ...
  isAllSelected() {
    if (JSON.parse(localStorage.getItem('nomCheck')!) == true)
      ...
      if (this.mfinal.length !== 0) {
          ...
          this.Compts[0].name.answers_products.forEach((prod: any) => {
            if (elm['reponse'] === this.Compts[0].name.name) {
              ...
              if (
               ...
               { this.cIndic.push(prod.product.id);
                console.log('push2',(this.ProductsEnabled.includes(prod.product.id) == true))

                this.ProductsEnabled.forEach((pe, index) => {

              if (pe==(prod.product.id)){
                this.ProductsEnabled.splice(index,1)
              }
            })
              }
        ...
        this.Compts[0].name.answers_products.forEach((prod: any) => {
          ...
          if ((prod.indication == true)&&(this.ProductsEnabled.includes(prod.product.id)==false)&&(this.cIndic.includes(prod.product.id)==false)) {
            ...
            console.log('push3',(this.ProductsEnabled.includes(prod.product.id) == true))
    ...
    if (event.checked == false) {
      ...
      this.Compts[0].name.answers_products.forEach((prod: any) => {
        this.cIndic.forEach((pe, index) => {
          ...
          this.products.forEach((pr,index)=>{
            if (pr.name ==pe){
      ...
      if (this.mfinal.length !== 0) {
        this.mfinal.forEach((element) => {
          ...
          element.product.forEach((ep: { indication: boolean; name: any }) => {
            ...
            if (
              ep.indication === true &&
              this.ProductsEnabled.includes(ep.name) == false
            ) {
              this.ProductsEnabled.push(ep.name);
            }
            if (
             ( ep.indication === false) &&
              (this.cIndic.includes(ep.name) == false)
            ) {
              this.cIndic.push(ep.name);
              console.log('push4',(this.ProductsEnabled.includes(ep.name) == true))
console.log(ep.name)
              this.ProductsEnabled.forEach((pe, index) => {
                if ( pe ==(ep.name)) {
                  this.ProductsEnabled.splice(index, 1);
